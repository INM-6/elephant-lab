...
def test_None_data(none_plotlyGraphFigure):
    ...
    assert np.allclose(none_plotlyGraphFigure.getXRange(), [0,0], rtol=1e-3)
...
def test_custom_x_range():
    ...
    assert not np.allclose(plotlyGraphFigure.getXRange(), [-5,4], rtol=1e-3)
    assert np.allclose(plotlyGraphFigure.getXRange(), [0,3], rtol=1e-3)
    assert np.allclose(plotlyGraphFigure.fig.data[0].x, [0,1,2,3], rtol=1e-3)
